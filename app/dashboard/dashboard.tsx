...
function Avatar() {
	return <span className="relative inline-block h-[38px] w-[38px] overflow-hidden rounded-full border-[3px] border-white bg-[#f49b58] shadow-[0_2px_9px_#da7b414d]" aria-label="Lay profile"><span className="absolute left-[7px] top-[3px] z-[1] h-[15px] w-[25px] rotate-[-12deg] rounded-[50%_60%_20%_15%] bg-[#4b2b28]" /><span className="absolute bottom-[3px] left-[8px] h-[26px] w-[21px] rounded-[45%_45%_48%_48%] bg-[#9a5c3e]"><span className="absolute bottom-[-1px] left-[3px] h-[12px] w-[15px] rounded-[4px_4px_12px_12px] bg-[#57312c]" /></span></span>;
}
...
function Sidebar() {
	return <aside className="flex flex-[0_0_265px] flex-col rounded-l-[22px] bg-white px-5 pb-[26px] pt-[21px] max-[1050px]:basis-[220px] max-[1050px]:px-[14px] max-[720px]:rounded-none max-[720px]:p-4">
		<div className="mb-[43px] ml-0.5 flex items-center gap-2.5 text-2xl font-extrabold tracking-[-.8px] max-[720px]:mb-[18px]"><span className="flex h-10 w-10 items-center justify-center [clip-path:polygon(25%_5%,75%_5%,100%_50%,75%_95%,25%_95%,0_50%)] bg-[#e7ae13] text-xl text-white">R</span><span>flex</span></div>
...
function MetricCard({ label, value, change, icon: Icon, direction }: (typeof metrics)[number]) {
	return <article className="flex min-h-[88px] items-center gap-[13px] rounded-[15px] bg-white px-4 py-[15px] max-[720px]:min-h-[78px]"><span className="inline-flex h-[43px] w-[43px] shrink-0 items-center justify-center rounded-full bg-[#f0efff] text-[#5c54e2]"><Icon size={19} /></span><div><p className="mb-[5px] text-xs text-[#82869a]">{label}</p><div className="flex items-baseline gap-[9px]"><strong className="text-[26px] tracking-[-.8px] max-[390px]:text-[23px]">{value}</strong><span className={`whitespace-nowrap text-[11px] font-bold ${direction === "up" ? "text-[#3bb68e]" : "text-[#ed7779]"}`}>{direction === "up" ? "↑" : "↓"} {change}</span></div></div></article>;
}

function SessionsCard() {
	return <article className="relative min-h-[402px] rounded-[15px] bg-white px-[15px] pb-[13px] pt-[17px] max-[720px]:min-h-[390px]"> 
    <div className="flex justify-between">
        <div>
            <p className="mb-[22px] text-[13px] font-bold text-[#171934] h-25">Online Store Sessions</p>

    <div className="flex items-center gap-[11px]">
        <span className="inline-flex h-[43px] w-[43px] items-center justify-center rounded-full bg-[#f0efff] text-[#5c54e2]">
            <Users size={18} /></span><div><span className="mb-0.5 block text-[11px] text-[#a0a3b4]">Visitors</span><strong className="block text-[25px] leading-none">68</strong></div>
            </div>
            </div>
            <button className="h-[33px] rounded-[22px] border border-[#ececf3] bg-white px-[13px] text-xs text-[#8d91a0]">View Report</button>
            </div>
...
function UpgradeCard() { return <article className="relative flex min-h-[133px] overflow-hidden rounded-[15px] bg-[#5147e9] p-[18px_15px] text-white max-[1050px]:min-h-[190px]"><div><p className="mb-1 text-lg font-bold">Need More Stats?</p><span className="text-[11px] text-[#bbb8ff]">Upgrade to pro for added benefits.</span><button className="mt-[18px] flex items-center gap-1.5 rounded-[20px] border-0 bg-[#33ba8e] px-4 py-[9px] text-[11px] font-bold text-white shadow-[0_6px_13px_#2722a34d]"><CircleArrowUp size={15} /> Go Pro Now</button></div><div className="absolute right-[13px] top-[-10px] text-3xl text-[#e8e9ff]"><span>↗</span><span className="ml-4">↑</span><span className="absolute bottom-[-100px] left-0">☁</span></div></article>; }

function ConversionCard() { return <article className="min-h-[249px] rounded-[15px] bg-white px-[15px] pt-[21px] max-[1050px]:min-h-[190px]"><p className="text-[13px] font-bold text-[#171934]">Conversion</p><div className="relative mx-auto mt-1 h-[132px] w-[82%] max-w-[260px] overflow-hidden rounded-t-[170px] bg-[conic-gradient(from_270deg_at_50%_100%,#7169ef_0deg_209deg,#e9e9f8_209deg_270deg,transparent_270deg)] after:absolute after:bottom-0 after:left-[16%] after:h-[83px] after:w-[68%] after:rounded-t-[145px] after:bg-white"><div className="absolute bottom-[13px] z-[1] w-full text-center"><strong className="block text-2xl">58,19%</strong><span className="text-[11px] font-bold text-[#36b88d]">↑ 3.5%</span></div></div><div className="mt-4 flex justify-between gap-3 border-t border-[#eeeef3] pt-3.5 text-[11px] text-[#9da0b0] max-[720px]:flex-wrap"><span><b className="mr-1.5 text-[17px] text-[#37b78e]">↙</b> Income <strong className="ml-1.5 text-[#42455a]">$542,317</strong></span><span><b className="mr-1.5 text-[17px] text-[#1d223c]">↗</b> Expenses <strong className="ml-1.5 text-[#42455a]">$497,456</strong></span></div></article>; }
